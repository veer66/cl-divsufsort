...
(defstruct (trbudget (:constructor make-trbudget (chance incval)))
  (chance 0 :type fixnum)
  (remain 0 :type fixnum)
  (incval 0 :type fixnum)
  (count 0 :type fixnum))

(defun trbudget-init (budget chance incval)
  (declare (type trbudget budget)
           (type fixnum chance incval))
  (setf (trbudget-chance budget) chance)
  (setf (trbudget-remain budget) incval)
  (setf (trbudget-incval budget) incval))

(defun trbudget-check (budget size)
  (declare (type trbudget budget)
           (type fixnum size))
  (cond ((<= size (trbudget-remain budget))
         (decf (trbudget-remain budget) size)
         t)
        ((zerop (trbudget-chance budget))
         (incf (trbudget-count budget) size)
         (values nil))
        (t
         (decf (trbudget-remain budget) size)
         (incf (trbudget-remain budget) (trbudget-incval budget))
         (decf (trbudget-chance budget))
         t)))
