...
(defun tandem-repeat-pivot (suffix-rank suffix-array first last)
  (declare (type fixnum first last)
           (type (simple-array fixnum *) suffix-rank suffix-array)
           (optimize (speed 3) (safety 0) (debug 0) (space 0)))
  (let* ((t-val (- last first))
         (middle (+ first (truncate t-val 2))))
    (cond ((<= t-val 512)
           (if (<= t-val 32)
               (tandem-repeat-median3 suffix-rank first middle (1- last))
               (let ((t2 (truncate t-val 4)))
                 (tandem-repeat-median5 suffix-rank first (+ first t2) middle (- (1- last) t2) (1- last)))))
          (t
           (let ((t3 (truncate t-val 8)))
             (setf first (tandem-repeat-median3 suffix-rank first (+ first t3) (+ first (* 2 t3))))
             (setf middle (tandem-repeat-median3 suffix-rank (- middle t3) middle (+ middle t3)))
             (setf last (tandem-repeat-median3 suffix-rank (- (1- last) (* 2 t3)) (- (1- last) t3) (1- last)))
             (tandem-repeat-median3 suffix-rank first middle last))))))

(defstruct (tandem-repeat-budget (:constructor make-tandem-repeat-budget (chance incval)))
  (chance 0 :type fixnum)
  (remain 0 :type fixnum)
  (incval 0 :type fixnum)
  (count 0 :type fixnum))

(defun tandem-repeat-budget-init (budget chance incval)
  (declare (type tandem-repeat-budget budget)
           (type fixnum chance incval))
  (setf (tandem-repeat-budget-chance budget) chance)
  (setf (tandem-repeat-budget-remain budget) incval)
  (setf (tandem-repeat-budget-incval budget) incval))

(defun tandem-repeat-budget-check (budget size)
  (declare (type tandem-repeat-budget budget)
           (type fixnum size))
  (cond ((<= size (tandem-repeat-budget-remain budget))
         (decf (tandem-repeat-budget-remain budget) size)
         t)
        ((zerop (tandem-repeat-budget-chance budget))
         (incf (tandem-repeat-budget-count budget) size)
         (values nil))
        (t
         (decf (tandem-repeat-budget-remain budget) size)
